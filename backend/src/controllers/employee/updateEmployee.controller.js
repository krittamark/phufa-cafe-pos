...
const updateEmployee = async (req, res, next) => {
  ...
  if (!empId) {
    return res
      .status(400)
      .json({message: 'Bad Request - Missing employee ID in path.'});
  }
  ...
  if (updateData.phoneNum && !/^\d{10}$/.test(updateData.phoneNum)) {
    return res.status(400).json({
      message:
        'Bad Request - Invalid Phone Number format (must be 9-10 digits).',
    });
  }
  ...
  try {
    ...
    try {
      ...
      let checkRows = [];
      if (
        typeof queryResultCheck === 'object' &&
        queryResultCheck !== null &&
        !Array.isArray(queryResultCheck)
      ) {
        // กรณีที่ 1: ผลลัพธ์เป็น Object เดี่ยวๆ (เจอข้อมูล)
        if (
          Object.prototype.hasOwnProperty.call(queryResultCheck, 'CitizenID')
        ) {
          checkRows = [queryResultCheck];
        } else {
          throw new Error(
            'Employee check returned an object without the expected CitizenID field.',
          );
        }
        ...
        if (queryResultCheck.length === 0) {
          ...
          Array.isArray(queryResultCheck[0]) &&
          queryResultCheck.length === 2 &&
          Array.isArray(queryResultCheck[1])
        ) {
          checkRows = queryResultCheck[0]; // [rows, fields]
        } else if (
          queryResultCheck.length > 0 &&
          typeof queryResultCheck[0] === 'object' &&
          queryResultCheck[0] !== null
        ) {
          checkRows = queryResultCheck; // [{...}]
        } else {
          console.error(
            '--- Unexpected array structure:',
            JSON.stringify(queryResultCheck),
          );
        }
        ...
        console.error(
          '--- Completely unexpected query result type:',
          typeof queryResultCheck,
        );
        throw new Error(
          'Unexpected database query result type for Employee check.',
        );
      }
      ...
      console.error(
        `Error checking employee existence for EmpID ${empId}:`,
        checkError,
      );
      await connection.rollback();
      connection.release();
          ...
          if (key === 'phoneNum' && value !== null && value !== undefined) {
            try {
              ...
              if (duplicateFound) {
                ...
                return res.status(400).json({
                  message:
                    'Bad Request - Phone number already registered by another person.',
                });
              }
            } catch (phoneCheckError) {
              console.error(
                `Error during PhoneNum uniqueness check for ${value}:`,
                phoneCheckError,
              );
    ...
    let finalRows = [];
    if (
      typeof updatedEmployeeRows === 'object' &&
      updatedEmployeeRows !== null &&
      !Array.isArray(updatedEmployeeRows)
    ) {
      if (
        Object.prototype.hasOwnProperty.call(updatedEmployeeRows, 'citizenId')
      ) {
      ...
      if (updatedEmployeeRows.length === 0) finalRows = [];
        ...
        Array.isArray(updatedEmployeeRows[0]) &&
        updatedEmployeeRows.length === 2 &&
        Array.isArray(updatedEmployeeRows[1])
      )
        finalRows = updatedEmployeeRows[0];
      else if (
        updatedEmployeeRows.length > 0 &&
        typeof updatedEmployeeRows[0] === 'object'
      )
        finalRows = updatedEmployeeRows;
      else {
        console.error('Unexpected array structure from SELECT query.');
      }
    } else if (
      updatedEmployeeRows === null ||
      updatedEmployeeRows === undefined
    ) {
      finalRows = [];
    } else {
    ...
    if (
      updatedEmployeeData &&
      typeof updatedEmployeeData.empSalary === 'string'
    ) {
    ...
    if (
      error.message === 'Failed to retrieve updated employee data after update.'
    ) {
      return res.status(500).json({
        message: 'Internal Server Error - Could not verify employee update.',
      });
    }
