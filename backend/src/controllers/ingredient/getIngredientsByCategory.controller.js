...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const category = await db.query('SELECT IngredientCategoryID FROM IngredientCategory WHERE IngredientCategoryID = ?', [ingredientCategoryId]);
    if (category.length === 0) {
      return res.status(404).json({ message: `Ingredient category with ID '${ingredientCategoryId}' not found.` });
    }
    ...
    console.error(`Error retrieving ingredients for category ID ${ingredientCategoryId}:`, error);
    res.status(500).json({ message: 'An unexpected error occurred while retrieving ingredients for the category.' });
  }
};