...
module.exports = async (req, res) => {
  ...
  try {
    const ingredient = await db.query(
      "SELECT IngredientID as ingredientId, Name as name, Quantity as quantity, Unit as unit, AdjustmentPrice as adjustmentPrice, CostPerUnit as costPerUnit, IngredientCategoryID as category FROM Ingredient WHERE IngredientID = ?",
      [ingredientId]
    );
    ...
    res
      ...
      .json({
        message:
          "An unexpected error occurred while retrieving the ingredient.",
      });
  }
};
