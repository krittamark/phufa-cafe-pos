...
const createCustomerController = async (req, res, next) => {
  ...
  if (!citizenId || !firstname || !lastname || !phoneNum) {
    return res.status(400).json({
      message:
        'Missing required fields: citizenId, firstname, lastname, phoneNum',
    });
  }
  if (citizenId.length !== 13 || !/^\d+$/.test(citizenId)) {
    return res
      .status(400)
      .json({message: 'Invalid Citizen ID format (must be 13 digits)'});
  }
  // ... (validation อื่นๆ)
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({message: 'Citizen ID or Phone Number already exists.'});
    }
    ...
    console.log(
      'Person inserted, affected rows:',
      personInsertResult.affectedRows,
    );
    ...
    console.log(
      'Customer inserted, affected rows:',
      customerInsertResult.affectedRows,
    );
    ...
    if (!newCustomerData || newCustomerData.length === 0) {
      console.error(
        `Failed to retrieve new customer data for ID: ${citizenId} after commit`,
      );
      return res
        .status(500)
        .json({message: 'Internal error retrieving created customer data.'});
    }
