...
const updateCustomerController = async (req, res, next) => {
    ...
    if (customerId.length !== 13 || !/^\d+$/.test(customerId)) {
        return res.status(400).json({ message: 'Invalid Customer ID format in URL (must be 13 digits)' });
   }
   ...
   // ... (validation อื่นๆ สำหรับ field ที่ส่งมา)
    ...
    try {
        ...
        if (!existingPerson || existingPerson.length === 0 || !existingPerson[0]?.CitizenID) { // ใช้ Optional chaining ?.
            await conn.release();
        ...
        if (updateData.phoneNum) {
            ...
            if (duplicatePhone && duplicatePhone.length > 0 && duplicatePhone[0]?.CitizenID) { // ใช้ Optional chaining ?.
                await conn.release();
        ...
        console.log(`Person updated for ${customerId}, affected rows:`, updateResult.affectedRows);
        ...
        if (!updatedCustomerData || updatedCustomerData.length === 0) {
             console.error(`Failed to retrieve updated customer data for ID: ${customerId}`);
             return res.status(500).json({ message: 'Internal error retrieving updated customer data.' });
         }
