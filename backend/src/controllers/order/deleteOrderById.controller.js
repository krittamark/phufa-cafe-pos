...
module.exports = async (req, res) => {
  ...
  const orders = await db.query(
    'SELECT OrderID, OrderStatus FROM `Order` WHERE OrderID = ?',
    [orderId],
  );
  ...
  if (orderToDelete.OrderStatus === 1 || orderToDelete.OrderStatus === true) {
    console.error(
      `Cannot delete order '${orderId}' because it has already been paid.`,
    );
    return res.status(400).json({
      message: `Cannot delete order '${orderId}' because it has already been paid.`,
    });
  }
  ...
  const deleteResult = await db.query('DELETE FROM `Order` WHERE OrderID = ?', [
    orderId,
  ]);
  ...
  if (deleteResult.affectedRows === 0) {
    ...
    console.error(
      `Order with ID '${orderId}' could not be deleted or was already deleted.`,
    );
    return res.status(500).json({
      message: `Order with ID '${orderId}' could not be deleted or was already deleted.`,
    }); // หรืออาจจะเป็น 500 ถ้าบ่งบอกถึง state ที่ไม่คาดคิด
  }
