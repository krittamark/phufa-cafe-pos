...
export default function OrderList({ onSelectOrder, selectedDate, orders, customers, employees, setOrders }: OrderListProps) {
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <td className="pl-6 py-4 text-sm text-gray-900">
                <span
                  className={`font-bold px-2 py-1 rounded text-white text-sm ${order.orderStatus ? 'bg-green-500' : 'bg-red-500'}`}
                >
              ...
              {order.orderStatus == true ? (
                ...
                <td className="px-4 text-sm text-gray-900 flex">
                  <button className="px-4 py-2 text-sm text-gray-900 hover:text-red-700 hover:bg-red-100 rounded-md text-right flex items-center justify-center gap-1 mt-1"
